...
This tutorial focuses on getting the UNO Q up and running using mostly **terminal tools** (ADB and SSH) and **Visual Studio Code** via the Remote-SSH extension. We deliberately skip Arduino App Lab's graphical interface (where it is possible) and instead focus on a professional, command-line-driven development workflow.
...
- **[Image Classification](../Image_Classification/README.md)**: Deploying a pre-trained image classifier using Edge Impulse and a USB camera, with MCU-side control of visual feedback (LEDs, displays).
