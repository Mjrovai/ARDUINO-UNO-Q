# Multimodal AI at the Edge: Giving a Small Model Eyes

![](./images/jpeg/cover.jpg)

> This chapter builds on [Generative AI at the Edge](../2-Gen_AI/README.md): it assumes you've already built `llama.cpp` from source, downloaded a Qwen3.5-0.8B GGUF, and are comfortable running `llama-server` by hand and calling it from Python. If any of that is unfamiliar, work through that chapter first.

---

## 1. Where We Left Off

In the previous chapter, the model read text. You ran `llama-cli` and `llama-server` from a single GGUF file (`Qwen_Qwen3.5-0.8B-Q8_0.gguf`), called it from Python, and got back answers to text prompts — all on the board, no cloud call.

Text is a strange interface for a lot of real problems, though. Think of a mosquito-breeding-site inspection: the thing you actually want to catch is *visual* — water sitting in an old tire, a plant saucer that never drains, a bucket left open after the rain. A health agent walking a neighborhood doesn't type numbers into a form. They look.
...
If you still have `llama-server` running from the previous chapter, stop it first (`Ctrl+C` in its terminal) — it's holding port 8081 and some RAM, and you want both back for this.
...
`--alias qwen3.5-0.8b` gives the server a stable name that doesn't depend on the GGUF filename or which model happens to be loaded. Client code always asks for `"model": "qwen3.5-0.8b"`; the server answers whether it's currently running text-only, with vision, or a completely different model behind that name. This is what makes the model-swapping pattern in [§8](#8-swapping-models-08b-for-speed-2b-for-depth) painless — the alias never changes.
...
## 8. Swapping Models: 0.8B for Speed, 2B for Depth

§7 established the trade-off. In practice, you don't pick one model and stick with it — you swap. Point a camera at a yard all day, and most frames are obviously nothing: dry ground, a wall, a parked car. A handful are ambiguous and worth a second, slower opinion. The natural pattern is a fast **screen** on the 0.8B, and a slower **deep read** on the 2B only for the frames that need it. The board can't run both at once comfortably (§7's RAM numbers), so "swap" here means *one server at a time on the same port*, not two processes running in parallel.

### Get the 2B model and its projector

Same models directory as before:

```bash
cd /home/arduino/models

# 2B language model — UD-Q4_K_XL, the quality/size sweet spot from Section 7
wget https://huggingface.co/unsloth/Qwen3.5-2B-GGUF/resolve/main/Qwen3.5-2B-UD-Q4_K_XL.gguf

# Its matching projector — do NOT reuse the 0.8B's mmproj (see Section 3, the n_embd mismatch trap)
wget https://huggingface.co/unsloth/Qwen3.5-2B-GGUF/resolve/main/mmproj-F16.gguf -O mmproj-F16_2B.gguf
```

### Two startup scripts

Wrap each model's launch command in its own script, so switching is one command instead of retyping flags:

```bash
nano ~/run-qwen-0.8b-vision.sh
```

```bash
#!/usr/bin/env bash
set -e
exec /home/arduino/llama.cpp/build/bin/llama-server \
  -m       /home/arduino/models/Qwen_Qwen3.5-0.8B-Q8_0.gguf \
...
set -e
exec /home/arduino/llama.cpp/build/bin/llama-server \
  -m       /home/arduino/models/Qwen3.5-2B-UD-Q4_K_XL.gguf \
  --mmproj /home/arduino/models/mmproj-F16_2B.gguf \
  --host 0.0.0.0 --port 8081 \
  -c 4096 -t 4 \
  -n 512 \
  --jinja \
...
```bash
chmod +x ~/run-qwen-0.8b-vision.sh ~/run-qwen-2b-vision.sh
```

Distinct aliases (`qwen3.5-0.8b` vs `qwen3.5-2b`) are deliberate here, unlike the single-model chapters before this one: since only one server runs at a time on port 8081, the alias echoed back in `choices[...]`'s response is your confirmation that the swap actually took effect, not just cosmetic. Your client code doesn't need to hardcode either name — a single-model `llama-server` ignores whatever `"model"` string a request sends, so the same Python script from Section 5 works unmodified against either script below. Reading the alias back is just how you'd sanity-check which one answered.

### Running one, then the other

Stop whichever server is currently running (`Ctrl+C`), then:

```bash
~/run-qwen-0.8b-vision.sh     # fast screen, ~1 min/image
# ...or...
~/run-qwen-2b-vision.sh       # deep read, ~4-5 min/image
```

If you want the server to keep running after you disconnect SSH — useful when a describe call takes several minutes and you don't want a dropped connection to kill it — run it inside `tmux` instead of directly:
...
Reattach later with `tmux attach -t vlm`, and stop the server with `Ctrl+C` inside that session.

### A rule of thumb

Use the smallest model that meets the task. The 0.8B is the right default for anything interactive or run over many frames — a live camera loop, a first-pass filter over a folder of field photos. Reach for the 2B only when a specific case needs the extra reasoning quality and the extra few minutes are acceptable: a second opinion on an ambiguous frame, or a final quality pass before a labeled dataset ships. In a real deployment, the natural design is a two-stage pipeline — the 0.8B screens everything, and only the frames it flags as uncertain get escalated to the 2B.
...
## 10. Reasoning Mode, and the Trap I Fell Into
...
Once the board is back up, SSH in as usual. Nothing starts automatically — you'll start `llama-server` by hand again, same as before.

The change from the earlier sections is small. Grab a frame, write it to disk at a sane resolution, and hand the path to the same `describe_image()` you already wrote. Let's start capturing an image (a frame) from my desk.
...
Section 9 showed the board producing a clean Question/Response/Reasoning row from a single image. Point the same code at a folder of field photos instead of one file, append each verdict to a JSONL file, and you have the start of a labeled dataset — generated offline, overnight, on a device that costs less than the camera that shot the photos.
...
### Feeding this into a dual-brain project

Everything in this chapter ran on the Linux side alone: a camera or a file, a description, a verdict, printed to a terminal. The [Gen_AI_Edge project](../4-Gen_AI_Edge/README.md) wires the *other* half of the UNO Q into a generative-AI application — an MCU sketch reading sensors, Bridge RPC connecting Python and the sketch, and an RGB LED reporting a verdict, with `llama-server` running as a proper systemd service. A button stands in there for a water-presence sensor. With a camera and the vision pathway from this chapter, that input can come from the model itself instead: the MCU triggers a capture on a timer or a motion sensor, the Linux side runs the describe-and-classify step you built here, and the verdict drives the LED. The Bridge RPC plumbing doesn't change — only the source of the signal does. That's the arc across the two chapters: the MCU senses, the MPU reasons, and the reasoning can include sight.

## 12. Conclusion

You now have the same UNO Q from the previous chapter, but looking at photos — or a live camera frame — and judging mosquito habitat, with a clear picture of what that costs: roughly a minute per image on the 0.8B, a couple more with reasoning on, about five on the 2B, all on four CPU cores, no cloud. You know why `--jinja` and the context size matter, why a frozen progress bar is usually the encoder doing honest work rather than a crash, and how to read `free -h` without scaring yourself.
...
- **One model, two modes.** The same weights that answered text prompts in the previous chapter run the vision describer here. The `mmproj` is the only addition, and there's no separate VL download to manage.
...
- The full dual-brain loop in the [Gen_AI_Edge project](../4-Gen_AI_Edge/README.md): MCU triggers the capture, MPU runs the verdict, LED responds — the camera replacing that project's button.
...
| GenAI Meets the Real World (dual-brain project, next step) | [4-Gen_AI_Edge/README.md](../4-Gen_AI_Edge/README.md) |
